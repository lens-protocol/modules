...
task('set-reference-module', 'sets the LZGatedReferenceModule on a post created by our sandbox profile')
  .addParam('hub')
  ...
  const res = await LZGatedReferenceModule__factory
    .connect(contracts.LZGatedReferenceModule, deployer)
    .gatedReferenceDataPerPub(SANDBOX_USER_PROFILE_ID, pubCount);

  console.log(`gatedReferenceDataPerPub(profileId: ${SANDBOX_USER_PROFILE_ID}, pubId: ${pubCount.toNumber()}) =>`, res);
});
