// Most of code taken from https://github.com/dmihal/eth-permit repository

import { BigNumber, BigNumberish } from '@ethersproject/bignumber';

export function toStringOrNumber(bigNumberish: BigNumberish): string | number {
  if (BigNumber.isBigNumber(bigNumberish)) {
    return bigNumberish.toString();
  } else if (bigNumberish instanceof String) {
    return bigNumberish as string;
  } else if (bigNumberish instanceof Number) {
    return bigNumberish as number;
  } else {
    return BigNumber.from(bigNumberish).toString();
  }
}
...
export const signData = async (provider: any, fromAddress: string, typeData: any): Promise<RSV> => {
  ...
  const result = await send(provider, 'eth_signTypedData_v4', [fromAddress, typeDataString]).catch(
    (error: any) => {
